...
- `dashboard/` — dashboard web server and scripts (`index.html`, `Caddyfile`, `valheim-status-collect.py`, `valheim-status.service`/`.timer`, `valheim-alert.py`, `valheim-alert.env` template, `valheim-world-scan.py`, `valheim-offsite-backup.sh`, `valheim-offsite.service`/`.timer`, `valheim-digest.py`, `valheim-digest.service`/`.timer`, `valheim-medals.py`, `valheim-medals-daily.service`/`.timer`, `valheim-medals-web.service`/`.timer` (the 10-minute feed behind the Hall of Fame page), `valheim-bot.py`/`valheim-bot.service`/`valheim-bot.env` (Hermodr), the restart feature (`valheim-restartd.py`/`.service`, `valheim-restart-exec.py`/`.service`/`.timer`/`.path`), PWA `manifest.webmanifest` + `icon.svg`/`icon-192.png`/`icon-512.png`, `install-dashboard.sh`, `PLAN-v3.md`, `PLAN-v4.md`, `PLAN-v5.md`).
