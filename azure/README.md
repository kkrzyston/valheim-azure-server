...
**Hermodr (Discord Q&A bot)** (`/usr/local/sbin/valheim-bot.py`, `valheim-bot.service`, added 2026-09-14) is a long-running Discord gateway bot (discord.py, its own venv at `/opt/hermodr/venv`) that answers questions in one Discord channel about the server, using the same on-disk data as everything else above -- `status.json`, the event log, and `valheim-medals.py`'s catalogue (imported live, never a second copy of the logic). It responds only when @-mentioned or when someone replies to one of its own messages, and only in the channel set by `HERMODR_CHANNEL_ID` (or, as a fallback, resolved once at startup by exact name from `HERMODR_CHANNEL_NAME`). Every question and every message it reads first passes a **channel allowlist check** -- before any parsing, before any AI call, before anything is logged -- so it is structurally incapable of answering (or leaking data) anywhere else; DMs are ignored entirely. It can answer: who's online, in-game day, bosses defeated, uptime, last raid; per-Viking hours/sessions/deaths/last-seen/longest-session/best-streak; the current holder of all ~27 medals for day/week/all-time, and how each one is calculated (sourced live from `valheim-medals.py`'s `CATALOG`, never a hand-copied list, so the explanations can't drift from the real logic); world exploration %, structures built, tombstones, tamed animals; and the tracking-start date, so it never claims "all-time" means longer ago than that. **Hermodr answers in Old Norse by default**: the model is instructed to write plain Latin-letter Old Norse (never runes itself), and a pure-Python transliterator (`to_futhark()`) prepends an Elder Futhark rendering of that same text before it is posted -- so every reply shows the runic line first, then the Old Norse sentence beneath it. Player names, medal names, the server address, and any number/date the model returns are backtick-wrapped by the model and left untouched by the transliterator, along with URLs, Discord mentions, and anything else with a digit in it (IPs, ports, timestamps). Ask for English explicitly in the same message ("in English", "translate that", "speak English", "á ensku", ...) and that one reply comes back in plain English with no runic line; the switch is a deterministic Python regex (`ENGLISH_RE`), not something the model decides for itself, since a prompt hard-instructed to never use English would otherwise refuse its own escape hatch. It cannot restart the server or change anything itself. Its one action beyond replying is to relay restart requests: it posts them to the approval channel and records which role holder pressed Approve or Deny (see **Restarting from the dashboard** below). Everything else it does is text posted back to the same channel, which is always sent with `allowed_mentions` set to none (so pings are inert at the Discord API level) with a second-layer strip of any literal `@everyone`/`@here` the model types anyway, capped at ~700 model tokens and truncated to Discord's 2000-char limit on a word boundary. A user's own message text is never folded into the system prompt -- it goes in as a separate untrusted `user` message, with the system prompt stating plainly that any instructions embedded in it must be ignored. Per-user rate limit (in-memory, resets on restart): 1 question per 10 s, 20 per hour; going over gets one brief reply, then silence. The model call itself uses the VM's system-assigned managed identity against an Azure AI Foundry endpoint (`HERMODR_AI_ENDPOINT`, default the `ai-valheim` project) -- **no API key stored anywhere**; **to swap the model, edit only `HERMODR_AI_MODEL`** in `/etc/valheim-bot.env` (default `hermodr-llm`) and restart the service. Setup: create a Discord application + bot user at <https://discord.com/developers/applications>, enable the **Message Content** privileged intent, invite it to the server with the `bot` scope and `Send Messages`/`Read Message History` permissions, then on the VM set `DISCORD_BOT_TOKEN` and `HERMODR_CHANNEL_ID` (or `HERMODR_CHANNEL_NAME`) in **`/etc/valheim-bot.env`** (moved out of `/etc/valheim-alert.env` on 2026-09-14, so the bot process no longer holds the Discord webhook credential) and `sudo systemctl enable --now valheim-bot`. The installer (`install-dashboard.sh`) creates the venv and installs the script and unit but deliberately does **not** enable or start it, since a fresh deploy has no token yet. Test locally with no bot token and no VM: `python3 valheim-bot.py --selftest` builds and prints the context (no Discord, no AI call); `python3 valheim-bot.py --ask "question"` builds the context and calls the AI endpoint once (only works on the VM, where managed identity exists; fails with a clear message anywhere else). Log: `/var/log/hermodr/hermodr.log` (moved 2026-09-14 when the bot stopped running as root; the unit's `LogsDirectory=hermodr` creates that directory owned by `valheim-bot`). The bot token is never written to it, redacted defensively even from tracebacks.

**Hermodr's game-knowledge corpus (added 2026-09-14)** gives Hermodr a second kind of answer
alongside the live server data above: game-mechanics questions ("what is Fenring weak to?", "how
much health does a Boar have?") answered from a wiki-derived reference corpus instead of the
model's own memory. The corpus comes from both community Valheim wikis and is built entirely
offline, never fetched live during a conversation -- `valheim-wiki-ingest.py` (its own venv,
dependencies pinned in `requirements-ingest.txt`: `mwparserfromhell` and `requests`, kept out of
Hermodr's own stdlib-only process on purpose) takes Fandom's published wiki dump for the base
~1,034 articles plus a delta re-fetch of anything edited since, and walks all ~1,179 articles on
the Valheim Wiki on Weird Gloop through its API (a fork of Fandom's wiki, so expect heavy
duplication rather than double the coverage), identifying itself with a descriptive User-Agent
and `maxlag=5` rather than a browser spoof, since Weird Gloop's `robots.txt` disallows automated
`api.php` access and names bots explicitly -- the owner decided to fetch anyway, on the terms
`PLAN-v6.md` sets out. Wikitext is parsed properly with `mwparserfromhell` (not regex), split one
record per page section, sanitized (no leftover `{{`/`[[` markup, no literal
`@everyone`/`@here`), and written to `wiki-records.jsonl`; `valheim-wiki-index.py` builds that
into a stdlib-only SQLite FTS5 index, `wiki.db`, that Hermodr's `search()` call queries at
question time. A game-knowledge answer is appended to the model's system prompt as its own
clearly labelled GAME KNOWLEDGE block, separate from the trusted SERVER CONTEXT block described
above, with every such answer required to cite the page it came from, both wikis' entries shown
side by side if they disagree, and a plain "unverified" flag when the corpus has nothing relevant
and the model falls back to its own Valheim knowledge. The corpus refreshes itself weekly
(`valheim-wiki-refresh.service`/`.timer`, Wednesdays ~04:15 local plus up to a 30-minute random
delay so a fleet of installs doesn't hit both wikis on the same clock tick, `Persistent=true` so
a missed week catches up at next boot) by running the ingest and then the index build back to
back as the unprivileged `valheim-bot` user, writing to `/var/lib/valheim-wiki/`; a failed run
leaves the previous `wiki.db` in place, since a stale index beats no index. Like every other timer
in this stack, **it ships installed but not enabled** -- a fresh deploy has no corpus yet, so the
first run should be an owner-supervised `sudo systemctl start valheim-wiki-refresh.service`
(watch `journalctl -u valheim-wiki-refresh` for the ingest's per-page skip log and the index
rebuild), and only once that comes back clean does `sudo systemctl enable --now
valheim-wiki-refresh.timer` turn on the weekly schedule. To run an ingest by hand against a small
sample instead of the full multi-thousand-page walk, the ingest script takes its own `--limit 20`
test flag, run in its dedicated venv with `requirements-ingest.txt` installed. Both source wikis'
licences require attribution -- Fandom's is CC BY-SA; Weird Gloop's is CC BY-SA 3.0 for revisions
before 2026-09-07 and **CC BY-NC-SA 3.0 (non-commercial) for revisions on or after it** -- stated
in full, in each licence's own required form, independently verified against both wikis'
licensing pages, in `dashboard/ATTRIBUTION.md`.
...
**The page has six views** (added 2026-09-14), navigated by a bar at the top and addressable by hash, so a link to one of them works: **Now** (`#/`) is the P0 answer -- up or down, who is in the world with session length and ping, the join address, the running version and update state, and a short activity feed; **The world** (`#/world`) has the map, bosses, what has been built and raids; **Vikings** (`#/vikings`) the roster, the 7-day timeline, who plays together and the heatmap; **Trends** (`#/trends`) the player-count and per-player ping charts; **Hall of fame** (`#/hall`) the medals, with tabs for yesterday/this week and the all-time records, rendered entirely from `medals.json` so adding a medal in `valheim-medals.py` lights it up here with no page change; and **The machine** (`#/server`) the meters, updates, backups, settings, snapshots link and the restart control. Only the visible view renders on each 30-second poll, which keeps the map and chart canvases off the hot path.

## Restarting from the dashboard

Added 2026-09-14. **This reverses the standing "never restart `valheim.service`" rule, in exactly one place.** `valheim-restart-exec.py`, running as root from `valheim-restart-exec.timer`, is now the only thing permitted to stop and start the game service, and only for a request a Discord role holder approved. Nothing else may touch it -- no other script, no sudoers rule, no dashboard code path. `PLAN-v5.md` in `dashboard/` is the full design; `PLAN-v4.md`'s hard rule carries the carve-out inline.

**How a restart happens.** Someone on the dashboard picks a reason from a fixed list and files a request. That writes one file and nothing else -- the web tier cannot restart anything. Hermodr posts the request in the approval channel with Approve and Deny buttons that only holders of the configured role can press. Once approved, the executor waits (no wait on an empty server, five minutes if anyone is online, announced in Discord), takes a verified world backup, stops the server, **confirms from the journal that the world actually flushed**, starts it again, and only calls it done when the server answers a real Steam query.

**Two things to be honest about.**

- **The requester is not authenticated.** `viking`/`hammerhead` is one shared password, the same one the game uses, and it is printed in this file. Anyone who can join the game can file a request. The Discord approval is the real control: it gives one named person accountability for each restart. It is *not* two-person control, because the approver may be the same human who filed it, and the Discord embed says so plainly.
- **Players in the game get no warning at all.** Vanilla Valheim gives a server no way to speak to players and this server runs no mods on purpose (they would break hands-off auto-update). The countdown reaches Discord and the dashboard only. That is why the grace period is five minutes rather than one.

**The rails, all of which must hold for a restart to proceed:** at least 30 minutes since the last one and at most 3 in 24 hours (`valheim.service` has `StartLimitBurst=5`/`StartLimitIntervalSec=600`, and exceeding that parks the unit until someone runs `reset-failed`); a `flock` on `/var/lock/valheim-world.lock` shared with the auto-updater and the nightly off-site backup, so a restart can never overlap either; a world backup that is actually verified (`tar -tzf`, entry count, size, sha256) before anything is stopped; `/home/valheim/data/.world-migrated` present and the unit's start condition satisfied -- if that marker is missing `systemctl start` silently succeeds and does nothing, so the executor refuses and **never re-creates it**; and if the stop times out (which means systemd SIGKILLed the process, possibly mid-save) it does **not** start again, because that would load a torn world and then overwrite the good backup.

If systemd has already parked the unit in `failed`, the executor clears it once with `reset-failed` and proceeds -- that state is exactly the outage this feature exists for, and the auto-updater deliberately does nothing about it. One clear per request, recorded in the audit log and shown on the page.

**Kill switches.** To make the whole feature inert with no effect on the game:

```bash
sudo systemctl disable --now valheim-restartd valheim-restart-exec.timer
```

Softer: set `accepting` to `false` in `/var/lib/valheim-restart/gate.json` to stop accepting new requests while letting one already in flight finish.

**It ships disarmed.** `valheim-restart-exec.service` carries `Environment=VR_DRY_RUN=1`, so it runs every check and posts to Discord (messages prefixed "Rehearsal:") but calls no systemctl. Arm it by removing that line and running `sudo systemctl daemon-reload`. Check `grep -c VR_DRY_RUN /etc/systemd/system/valheim-restart-exec.service` -- 1 means still rehearsing, 0 means armed.

**Setup it needs.** A separate, role-restricted Discord channel for approvals, and a role whose holders may approve. Put the ids in `/etc/valheim-bot.env` as `HERMODR_GUILD_ID`, `RESTART_APPROVAL_CHANNEL_ID` and `RESTART_APPROVER_ROLE_ID` (right-click with Developer Mode on to copy an id). **The feature fails closed**: with any of them unset the bot posts nothing and writes no verdicts. Note that anyone with Manage Roles in that guild can grant themselves the approver role, so Discord server-admin implies restart authority.

**Where things are.** Spool `/var/lib/valheim-restart/` (`requests/` written by `valheim-restartd`, `inbox/` and the state written by root, `verdicts/` written by the bot; the directory modes are the security model and `security-check.sh` asserts them). Audit log `/var/log/valheim-restart.jsonl` plus `journalctl -t valheim-restart`. The page reads `/var/www/valheim/restart-state.json`, which is `0640 root:caddy` because it carries the page's CSRF token.
...
- `dashboard/` — dashboard web server and scripts (`index.html`, `Caddyfile`, `valheim-status-collect.py`, `valheim-status.service`/`.timer`, `valheim-alert.py`, `valheim-alert.env` template, `valheim-world-scan.py`, `valheim-offsite-backup.sh`, `valheim-offsite.service`/`.timer`, `valheim-digest.py`, `valheim-digest.service`/`.timer`, `valheim-medals.py`, `valheim-medals-daily.service`/`.timer`, `valheim-medals-web.service`/`.timer` (the 10-minute feed behind the Hall of Fame page), `valheim-bot.py`/`valheim-bot.service`/`valheim-bot.env` (Hermodr), its wiki knowledge base (`valheim-wiki-ingest.py`, `requirements-ingest.txt`, `valheim-wiki-index.py`, `valheim-wiki-refresh.service`/`.timer`, `ATTRIBUTION.md`), the restart feature (`valheim-restartd.py`/`.service`, `valheim-restart-exec.py`/`.service`/`.timer`/`.path`), PWA `manifest.webmanifest` + `icon.svg`/`icon-192.png`/`icon-512.png`, `install-dashboard.sh`, `PLAN-v3.md`, `PLAN-v4.md`, `PLAN-v5.md`, `PLAN-v6.md`).
