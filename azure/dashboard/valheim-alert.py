...
# From /etc/valheim-server.env (see valheim-status.service's EnvironmentFile=).
...
if "bosses" not in st:
    ...
    for key, defeated in bosses_now.items():
        if defeated and not st["bosses"].get(key):
            ...
            post(f"{display} has fallen", f"{display} has been defeated on {WORLD_NAME}. Existence is pain, but a little less of it now.", "torch", everyone=True)
...
for row in stats:
    ...
    for t in HOUR_MILESTONES:
        if prev["hours"] < t <= hours:
            post(f"{name} reaches {t} hours", f"{name} has spent {t}+ hours in {WORLD_NAME}.", "ok")
