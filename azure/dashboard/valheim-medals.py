#!/usr/bin/env python3
"""valheim-medals.py -- Discord "medals" for Vancouver Island, computed from the dashboard's
...
CLI modes:
  ...
  --daily           "Yesterday on Vancouver Island" -- skips entirely if nobody played
...
def daily_embed(now=None):
    ...
    embed = {"title": "Yesterday on Vancouver Island", "description": clamp_desc(desc),
             "color": COLORS["ok"], "footer": {"text": "1g49ye on Vancouver Island · daily medals"}}
    if DASH:
        embed["url"] = DASH
    return embed
...
def weekly_embed(now=None):
    ...
    return {"title": "Medals this week", "description": clamp_desc(desc), "color": COLORS["gold"],
            "footer": {"text": "1g49ye on Vancouver Island · medals"}}
...
def board_embed():
    ...
    fields = fields[:24]
    embed = {"title": "Vancouver Island Hall of Fame",
...
def record_embed(entry, res, status):
    ...
    return {"title": f"New record: {entry['name']}",
            "description": f"{line}\n\nA new all-time high on Vancouver Island, {since_date_str(status)}.",
            "color": COLORS["gold"], "footer": {"text": "1g49ye on Vancouver Island · medals"}}
