...
# From /etc/valheim-server.env, same as every other script (see valheim-bot.py's copy of this
# pattern) -- valheim-restart-exec.service also needs EnvironmentFile=-/etc/valheim-server.env
# alongside its existing -/etc/valheim-alert.env, since that's where these two live.
SERVER_NAME = env("SERVER_NAME", "").strip() or "Valheim Server"
WORLD_NAME = env("WORLD_NAME", "").strip() or "Dedicated"
...
class Webhook:
    ...
    def _body(self, title, text, level, fields=None):
        ...
        embed = {"title": title, "description": text, "color": self.COLORS.get(level, 0x93A8AB),
                 "footer": {"text": f"{SERVER_NAME} on {WORLD_NAME}"}}
        if fields:
            embed["fields"] = fields
        if self.dash:
            embed["url"] = self.dash
...
class Executor:
    ...
    def write_inbox(self, req, expires):
        ...
        doc = {
            ...
            "notes": [
                "The requester is not authenticated: the dashboard's join password is shared with "
                "everyone in the hall, so this could be anyone who has it.",
    ...
    def verify_tarball(self, path):
        ...
        try:
            ...
            if info["bytes"] < 1 << 20:                  # a real <WORLD_NAME> save is tens of MB
