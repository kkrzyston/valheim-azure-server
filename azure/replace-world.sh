...
# Replaces the live "Vancouver Island" world with the copy uploaded to /home/azureuser/Vancouver Island.
# Steps: verify upload against /home/azureuser/world.sha256 -> graceful stop (saves) -> archive current world -> install -> start.
set -euo pipefail
SRC="/home/azureuser/Vancouver Island"
DST_DIR="/home/valheim/data/worlds_local"
DST="$DST_DIR/Vancouver Island"
SEED="uEA2lEA5YC"
...
if [ -d "$DST" ]; then
  ...
  tar -czf "/home/valheim/backups/vancouver-island-pre-replace-$TS.tgz" -C "$DST_DIR" "Vancouver Island"
  mv "$DST" "$DST_DIR/Vancouver Island_replaced-$TS"
  chown -R valheim:valheim /home/valheim/backups
  echo "current world (save ${cursave:-?}) archived: /home/valheim/backups/vancouver-island-pre-replace-$TS.tgz"
  echo "and kept on disk as: $DST_DIR/Vancouver Island_replaced-$TS"
else
